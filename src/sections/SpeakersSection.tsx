...
const SpeakersSection: React.FC<SpeakersSectionProps> = ({ featured = false }) => {
  ...
  const speakers = featured ? pageData.SpeakersSection.speakers.slice(0, MAX_FEATURED) : pageData.SpeakersSection.speakers

  return (
    <section className={`speakers-section w-full ${featured ? 'featured' : 'speakers'}`}>
      <div className={'w-full'}>
        <div className={'min-h-[2200px] p-4 py-20 md:pb-[120px] md:pt-[120px]'}>
          <h2 className={'mb-5 text-center text-[32px] font-bold leading-none tracking-[-0.96px] text-black text-white lg:text-[68px] lg:tracking-[-2.04px]'}>
